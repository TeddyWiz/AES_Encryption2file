...
#pragma inline
void *myMemcpy(void *__restrict s1, const void *__restrict s2, size_t n)
...
int AES_CBC_encrypt_File(uint8_t *buf, uint8_t *fileName)
{
  ...
  AES_CBC_encrypt_bufferEx(&ctx, buf, EncBuff, (size));
  #if AES_DEBUG
  printf("\nEncorded: \r\n");
  for (i=0; i<(size); i++) printf(" %02X", EncBuff[i]&0x000000FF);
  #endif
  fp = fopen(fileName,"w");
  fwrite(EncBuff,size+10 , 1, fp);
  fclose(fp);
  free(EncBuff);
  return 0;
}
...
char *AES_CBC_decrypt_File(uint8_t *buf, uint8_t *fileName)
{
  ...
  #if AES_DEBUG
  printf("\nread data : \n");
  for(i=0; i<size; i++) printf("%02X ", buf[i]&0x000000FF);
  #endif
